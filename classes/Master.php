...
Class Master extends DBConnection {
	...
	function save_category() {
		...
		// Build the data string for the query
		...
		if (isset($_POST['description'])) {
			if (!empty($data)) $data .= ",";
			$data .= " `description`='" . $this->conn->real_escape_string(addslashes(htmlentities($description))) . "' ";
		}
	
		// Check for duplicate category based on type
		$check = $this->conn->query("SELECT * FROM `category_list` WHERE `type` = '{$type}' AND delete_flag = 0 " . (!empty($id) ? " AND id != {$id} " : ""));
		...
		if ($save) {
			...
			// Capture and return the error for debugging
			$resp['status'] = 'failed';
			$resp['msg'] = "Error: " . $this->conn->error . " [{$sql}]"; // Display SQL error
		}
	...
	function save_booking(){
		if(empty($_POST['id'])){
			...
			$_POST['client_id'] = $this->settings->userdata('id');
			$_POST['ref_code'] = $prefix . $code;
			$_POST['Payment_status'] = 'Pending';  // Default value
			$_POST['Visible_status'] = 'True';     // Default value
		}
	...
	public function update_payment_status() {
		extract($_POST);
		$update = $this->conn->query("UPDATE `booking_list` SET `Payment_status` = '{$Payment_status}' WHERE id = '{$id}'");
		if ($update) {
	...
	function update_booking_statusa(){
		extract($_POST);
		$update = $this->conn->query("UPDATE `booking_list` set `status` = '{$status}' where id = '{$id}' ");
		if($update){
			$resp['status'] = 'success';
